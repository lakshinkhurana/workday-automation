...
class WorkdayFormScraper:
    ...
    async def run(self) -> ExtractionResults:
        ...
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=os.getenv('HEADLESS_STATE',False).lower() == 'true')
            ...
            try:
                ...
                if account_success:
                    ...
                    # Phase 6: Upload resume for autofill (no manual navigation needed)
                    print("\n📍 Phase 6: Uploading Resume for Autofill")
                    await self._upload_resume_for_autofill(page)
                else:
        ...
        for selector in job_title_selectors:
            try:
                ...
                if job_elements:
                    ...
                    await first_job.click(force=True)
    ...
    async def _click_apply_button(self, page: Page) -> bool:
        ...
        # Step 2: Look for and click Autofill with Resume button (preferred) or Apply Manually as fallback
        print("  🔍 Looking for Autofill with Resume button...")
        
        # First priority: Autofill with Resume options
        autofill_resume_selectors = [
        ...
        # Try autofill with resume first
        for selector in autofill_resume_selectors:
            try:
                element = await page.wait_for_selector(selector, timeout=5000, state='visible')
                if element:
                    button_text = await element.inner_text()
                    print(f"  ✅ Found Autofill with Resume button: '{button_text}'")
                    await element.click(force=True)
                    print("  🖱️ Clicked Autofill with Resume button")
                    
                    # Wait for resume upload/autofill page to load
                    await page.wait_for_load_state("networkidle", timeout=10000)
                    await asyncio.sleep(2)
                    return True
            except:
        ...
        print("  ⚠️ No Autofill with Resume button found, trying Apply Manually as fallback...")
        
        # Fallback: Apply Manually options
        ...
        print("  ⚠️ No Autofill with Resume or Apply Manually button found, but Apply was clicked")
    ...
    async def _submit_account_form(self, page: Page) -> bool:
        """Submit the account creation form using stable method"""
        ...
        for selector in submit_selectors:
            try:
                ...
                if element:
                    ...
                    if is_disabled:
                        print(f"    ⚠️ Submit button is disabled: {selector}")
                        continue
                    
                    # Wait for element to be stable (prevent movement issues)
                    await asyncio.sleep(1)
                    
                    # Scroll element into view to prevent movement
                    await element.scroll_into_view_if_needed()
                    await asyncio.sleep(0.5)
                    
                    # Single, stable click approach
                    await element.click(force=True)
    ...
    async def _upload_resume_for_autofill(self, page: Page):
        """Upload resume file to trigger autofill functionality"""
        print("  � Loocking for resume upload button...")
        ...
        try:
            ...
            # Look for the "Select file" button for resume upload
            ...
            for selector in select_file_selectors:
                try:
                    element = await page.wait_for_selector(selector, timeout=5000, state='visible')
                    if element:
                        button_text = await element.inner_text() if selector.startswith('button') else 'File Input'
                        print(f"  ✅ Found resume upload element: '{button_text}' using selector: {selector}")
                        
                        # Get resume file path from environment
                        resume_path = os.getenv('RESUME_PATH', '')
                        if not resume_path:
                            print("  ⚠️ RESUME_PATH environment variable not set")
                            break
                        ...
                        # Upload the file using the same reliable method as DirectFormFiller
                        if selector == 'button[data-automation-id="select-files"]':
                            # Use the robust file chooser method for the main button
                            upload_success = await self._upload_with_file_chooser(page, element, resume_path)
                            ...
                            # Button that opens file dialog - use file chooser method
                            upload_success = await self._upload_with_file_chooser(page, element, resume_path)
                            if not upload_success:
                                continue
                        
                        print(f"  ✅ Resume uploaded successfully: {os.path.basename(resume_path)}")
                        resume_uploaded = True
                        
                        # Wait for autofill to process
                        print("  ⏳ Waiting for autofill to process resume...")
                        await asyncio.sleep(5)
                        await page.wait_for_load_state("networkidle", timeout=15000)
                        print("  ✅ Autofill processing complete")
                        
                        # Press Continue button after autofill
                        continue_success = await self._press_continue_button(page)
                        if continue_success:
                            print("  ✅ Continue button pressed successfully")
                            
                            # Fill only the specific fields that resume doesn't handle using DirectFormFiller
                            print("  📝 Filling remaining fields on My Information page...")
                            await self._fill_remaining_fields_with_direct_filler(page)
                        else:
                            print("  ⚠️ Continue button not found or could not be pressed")
            ...
            if not resume_uploaded:
                print("  ⚠️ Could not find resume upload button or upload failed")
                # Continue anyway as the form might still be accessible
            
        except Exception as e:
            self.errors.append(f"Error uploading resume: {str(e)}")
            print(f"  ❌ Error uploading resume: {str(e)}")
    
    async def _upload_with_file_chooser(self, page: Page, element, resume_path: str) -> bool:
        """Upload file using the robust file chooser method from DirectFormFiller"""
        try:
            print(f"    🔍 Using file chooser method for upload...")
            
            # Set up file chooser handler before clicking
            async def handle_file_chooser(file_chooser):
                await file_chooser.set_files(resume_path)
                print(f"    ✅ File selected via chooser: {os.path.basename(resume_path)}")
            ...
            await element.click(force=True)
            print("    🖱️ Clicked upload element")
            
            # Wait for file dialog and upload (same timing as DirectFormFiller)
            await asyncio.sleep(3)
            
            # Remove the event listener
            page.remove_listener("filechooser", handle_file_chooser)
            
            print("    ✅ File chooser upload completed")
            return True
            
        except Exception as e:
            print(f"    ❌ Error with file chooser upload: {str(e)}")
            return False
    
    async def _press_continue_button(self, page: Page) -> bool:
        """Press Continue button after resume autofill completes"""
        print("    🔍 Looking for Continue button...")
        
        # Wait a moment for autofill to fully complete
        await asyncio.sleep(2)
        ...
        for selector in continue_selectors:
            try:
                ...
                if element:
                    ...
                    is_disabled = await element.is_disabled()
                    if is_disabled:
                        print(f"    ⚠️ Continue button is disabled: {selector}")
                        continue
                    
                    button_text = await element.inner_text()
                    print(f"    ✅ Found Continue button: '{button_text}' using selector: {selector}")
                    
                    # Scroll element into view to prevent movement
                    await element.scroll_into_view_if_needed()
                    await asyncio.sleep(0.5)
                    
                    # Use force click for reliability
                    await element.click(force=True)
                    print(f"    🖱️ Clicked Continue button")
    ...
    async def _fill_remaining_fields_with_direct_filler(self, page: Page):
        """Fill remaining fields using DirectFormFiller's proven methods"""
        print("    📝 Using DirectFormFiller to fill remaining My Information fields...")
        
        try:
            # Wait for page to load with extended timeout
            await page.wait_for_load_state("networkidle", timeout=15000)
            await asyncio.sleep(5)  # Extended wait for My Information page to fully load
            print("    ⏳ Waiting for My Information fields to become available...")
            
            # Create DirectFormFiller instance
            direct_filler = DirectFormFiller()
            
            # Use DirectFormFiller's debug method to see what's on the page
            await direct_filler._debug_page_fields(page)
            
            # Use DirectFormFiller's main filling method
            filled_count = await direct_filler.fill_page_by_automation_id(page)
            
            if filled_count > 0:
                print(f"    ✅ DirectFormFiller filled {filled_count} remaining fields")
            else:
                print("    ⚠️ No additional fields found to fill - resume autofill may have handled everything")
            
            # Press Save and Continue button after filling fields
            print("    💾 Looking for Save and Continue button...")
            save_continue_success = await self._press_save_and_continue(page)
            
            if save_continue_success:
                print("    ✅ Save and Continue pressed successfully")
                # Wait for next page to load
                await page.wait_for_load_state("networkidle", timeout=15000)
                await asyncio.sleep(3)
                print("    ✅ Proceeded to next page")
                
                # Extract JSON and process the new page
                print("    📄 Processing next page...")
                await self._process_page_extract_and_continue(page)
            else:
                print("    ⚠️ Save and Continue button not found or could not be pressed")
            
        except Exception as e:
            print(f"    ❌ Error using DirectFormFiller: {str(e)}")
    
    async def _press_save_and_continue(self, page: Page) -> bool:
        """Press Save and Continue button using DirectFormFiller's proven method"""
        print("    🔍 Looking for Save and Continue button...")
        ...
        for selector in save_continue_selectors:
            try:
                ...
                if element:
                    ...
                    is_disabled = await element.is_disabled()
                    if is_disabled:
                        print(f"    ⚠️ Save and Continue button is disabled: {selector}")
                        continue
                    
                    button_text = await element.inner_text()
                    print(f"    ✅ Found Save and Continue button: '{button_text}' using selector: {selector}")
                    
                    # Scroll element into view to prevent movement
                    await element.scroll_into_view_if_needed()
                    await asyncio.sleep(0.5)
                    
                    # Use force click for reliability (same as DirectFormFiller approach)
                    await element.click(force=True)
        ...
        print("    ⚠️ Save and Continue button not found")
        return False
    
    async def _process_page_extract_and_continue(self, page: Page):
        """Extract JSON from current page, fill required fields, and press Save and Continue"""
        print("    📄 Processing page: Extract JSON and Continue...")
        ...
        try:
            ...
            await self._extract_current_page_json(page, current_page_title)
            
            # Fill any required fields on this page
            await self._fill_required_fields_on_current_page(page)
            
            # Press Save and Continue to move to next page
            print("    💾 Looking for Save and Continue button...")
            save_continue_success = await self._press_save_and_continue(page)
            ...
            if save_continue_success:
                ...
                print("    ⚠️ Save and Continue button not found - application may be complete")
                # Try to extract JSON from final page
                final_page_title = await page.title()
                await self._extract_current_page_json(page, final_page_title)
    ...
    async def _extract_current_page_json(self, page: Page, step_name: str):
        ...
        print(f"    📋 Extracting JSON from current page: {step_name}...")
        ...
        try:
            ...
            page_info = PageInfo(
                ...
                path=current_url.replace(self.tenant_url, '') or '/',
                title=page_title,
                page_type=self._classify_page_type(current_url, page_title),
                visited=True
            )
            ...
            print(f"    ✅ Extracted {len(page_forms)} form elements from: {page_title}")
            
            # Save intermediate results after each page
            results = await self._create_results()
            await self._save_results(results)
            print(f"    💾 JSON saved for page: {page_title}")
            
        except Exception as e:
            print(f"    ❌ Error extracting JSON from page: {str(e)}")
    
    async def _fill_required_fields_on_current_page(self, page: Page):
        """Fill required fields on current page using DirectFormFiller and aria-required detection"""
        print("    📝 Filling required fields on current page...")
        
        try:
            # Check if this is the Voluntary Disclosures page
            page_title = await page.title()
            page_url = page.url
            
            if self._is_voluntary_disclosures_page(page_title, page_url):
                print("    📊 Detected Voluntary Disclosures page - using specialized handling...")
                await self._handle_voluntary_disclosures_page(page)
                return
            
            # Check if this is My Experience page to skip self identity date elements
            is_experience_page = self._is_my_experience_page(page_title, page_url)
            
            # For other pages, use DirectFormFiller to fill known fields
            direct_filler = DirectFormFiller()
            filled_count = await direct_filler.fill_page_by_automation_id(page)
            
            if filled_count > 0:
                print(f"    ✅ DirectFormFiller filled {filled_count} fields")
            
            # Then, check for any aria-required fields that might still need filling
            required_elements = await page.query_selector_all('[aria-required="true"]')
            
            if required_elements:
                print(f"    🔍 Found {len(required_elements)} aria-required fields")
                
                additional_filled = 0
                for i, element in enumerate(required_elements):
                    try:
                        # Check if already filled
                        element_type = await element.get_attribute('type') or await element.evaluate('el => el.tagName.toLowerCase()')
                        current_value = await element.input_value() if element_type in ['text', 'email', 'tel', 'password'] else await element.get_attribute('value')
                        
                        if current_value and current_value.strip():
                            continue  # Skip if already filled
                        
                        # Get element attributes for value determination
                        element_id = await element.get_attribute('id') or f'required-field-{i}'
                        element_name = await element.get_attribute('name') or ''
                        element_automation_id = await element.get_attribute('data-automation-id') or ''
                        
                        # Skip self identity date elements on My Experience page
                        if is_experience_page and self._is_self_identity_date_element(element_id, element_name, element_automation_id):
                            print(f"    ⏭️ Skipping self identity date element on My Experience page: {element_id}")
                            continue
                        
                        # Determine what value to fill
                        fill_value = await self._determine_required_field_value(element_id, element_name, element_automation_id, element_type)
                        
                        if fill_value:
                            success = await self._fill_required_field(page, element, element_type, fill_value, element_id)
                            if success:
                                additional_filled += 1
                                print(f"    ✅ Filled aria-required field '{element_id}': {fill_value}")
                                
                    except Exception as e:
                        print(f"    ⚠️ Error processing aria-required field {i+1}: {str(e)}")
                        continue
                
                if additional_filled > 0:
                    print(f"    ✅ Filled {additional_filled} additional aria-required fields")
            
            total_filled = filled_count + (additional_filled if 'additional_filled' in locals() else 0)
            if total_filled > 0:
                print(f"    ✅ Total fields filled on this page: {total_filled}")
            else:
                print("    ℹ️ No fields needed filling on this page")
            
        except Exception as e:
            print(f"    ❌ Error filling required fields: {str(e)}")
    
    def _is_voluntary_disclosures_page(self, page_title: str, page_url: str) -> bool:
        """Check if current page is the Voluntary Disclosures page"""
        voluntary_keywords = [
            'voluntary', 'disclosure', 'disclosures', 'eeo', 'equal opportunity',
            'ethnicity', 'gender', 'veteran', 'diversity', 'demographics'
        ]
        
        combined_text = f"{page_title} {page_url}".lower()
        
        return any(keyword in combined_text for keyword in voluntary_keywords)
    
    def _is_my_experience_page(self, page_title: str, page_url: str) -> bool:
        """Check if current page is the My Experience page"""
        experience_keywords = [
            'experience', 'my experience', 'work experience', 'employment',
            'work history', 'career', 'resume', 'cv'
        ]
        
        combined_text = f"{page_title} {page_url}".lower()
        
        return any(keyword in combined_text for keyword in experience_keywords)
    
    def _is_self_identity_date_element(self, element_id: str, element_name: str, automation_id: str) -> bool:
        """Check if an element is a self identity date element that should be skipped on My Experience page"""
        combined_identifiers = f"{element_id} {element_name} {automation_id}".lower()
        
        # Self identity date patterns to skip on My Experience page
        ...
        return any(pattern in combined_identifiers for pattern in self_identity_patterns)
    
    async def _handle_voluntary_disclosures_page(self, page: Page):
        """Handle Voluntary Disclosures page specifically - extract JSON, fill fields, tick checkbox, save and continue"""
        print("    📊 Processing Voluntary Disclosures page...")
        
        try:
            # Extract JSON from this page first
            page_title = await page.title()
            await self._extract_current_page_json(page, page_title)
            
            # Use DirectFormFiller's specialized voluntary disclosures method
            direct_filler = DirectFormFiller()
            success = await direct_filler.handle_voluntary_disclosures(page)
            
            if success:
                print("    ✅ Voluntary disclosures filled successfully")
            else:
                print("    ⚠️ Some voluntary disclosure fields may not have been filled")
            
            # Look for and tick any checkboxes (terms, conditions, etc.)
            await self._tick_voluntary_disclosures_checkbox(page)
            
            # Press Save and Continue
            print("    💾 Looking for Save and Continue button on Voluntary Disclosures page...")
            save_success = await self._press_save_and_continue(page)
            
            if save_success:
                print("    ✅ Voluntary Disclosures page completed successfully")
            else:
                print("    ⚠️ Could not find Save and Continue button on Voluntary Disclosures page")
            
        except Exception as e:
            print(f"    ❌ Error handling Voluntary Disclosures page: {str(e)}")
    
    async def _tick_voluntary_disclosures_checkbox(self, page: Page):
        """Find and tick checkboxes on Voluntary Disclosures page"""
        print("    🔲 Looking for checkboxes on Voluntary Disclosures page...")
        
        checkbox_selectors = [
            'input[type="checkbox"]',
            'input[data-automation-id*="checkbox"]',
            'input[data-automation-id*="terms"]',
            'input[data-automation-id*="agree"]',
            'input[data-automation-id*="consent"]',
            'input[data-automation-id*="acknowledge"]'
        ]
                ...
                for checkbox in checkboxes:
                    try:
                        ...
                        if is_visible and not is_checked:
                            # Get checkbox label or nearby text for context
                            checkbox_id = await checkbox.get_attribute('id') or 'unknown'
                            
                            # Look for associated label
                            label_text = 'unknown'
                            if checkbox_id != 'unknown':
                            ...
                            checkboxes_ticked += 1
                            print(f"    ✅ Ticked checkbox: {label_text[:50]}...")
                            
                    except Exception as e:
                        print(f"    ⚠️ Error processing checkbox: {str(e)}")
                        continue
        ...
        if checkboxes_ticked > 0:
            print(f"    ✅ Ticked {checkboxes_ticked} checkboxes on Voluntary Disclosures page")
        else:
            print("    ℹ️ No checkboxes found or all were already checked")
    
    async def _fill_required_fields_on_next_page(self, page: Page):
        """Check all form elements for aria-required='true' and fill them with .env data"""
        print("    🔍 Scanning for required fields on next page...")
        ...
        try:
            ...
            # Find all elements with aria-required="true"
            required_elements = await page.query_selector_all('[aria-required="true"]')
            
            if not required_elements:
                print("    ℹ️ No required fields found on this page")
                return
            
            print(f"    📋 Found {len(required_elements)} required fields")
            ...
            for i, element in enumerate(required_elements):
                try:
                    ...
                    print(f"      🔍 Required field {i+1}: id='{element_id}', name='{element_name}', type='{element_type}', automation-id='{element_automation_id}'")
                    
                    # Skip if already filled
                    current_value = await element.input_value() if element_type in ['text', 'email', 'tel', 'password'] else await element.get_attribute('value')
                    if current_value and current_value.strip():
                        print(f"      ✅ Field already filled: {element_id}")
                        continue
                    
                    # Determine what value to fill based on field characteristics
                    fill_value = await self._determine_required_field_value(element_id, element_name, element_automation_id, element_type)
                    ...
                    if fill_value:
                        ...
                        if success:
                            filled_count += 1
                            print(f"      ✅ Filled required field '{element_id}': {fill_value}")
                        else:
                            print(f"      ⚠️ Failed to fill required field: {element_id}")
                    else:
                        print(f"      ⚠️ No suitable value found for required field: {element_id}")
                        
                except Exception as e:
                    print(f"      ❌ Error processing required field {i+1}: {str(e)}")
                    continue
            
            print(f"    ✅ Filled {filled_count} required fields on next page")
            
            # If we filled any fields, try to save and continue again
            if filled_count > 0:
                print("    💾 Looking for Save and Continue button after filling required fields...")
                await asyncio.sleep(2)  # Wait for any dynamic updates
                save_success = await self._press_save_and_continue(page)
                if save_success:
                    print("    ✅ Save and Continue pressed after filling required fields")
                    # Wait for next page and recursively check for more required fields
                    await page.wait_for_load_state("networkidle", timeout=15000)
                    await asyncio.sleep(3)
                    await self._fill_required_fields_on_next_page(page)
                else:
                    print("    ⚠️ Save and Continue not found after filling required fields")
            
        except Exception as e:
            print(f"    ❌ Error scanning for required fields: {str(e)}")
    
    async def _determine_required_field_value(self, field_id: str, field_name: str, automation_id: str, field_type: str) -> str:
        """Determine what value to use for a required field based on its characteristics"""
        
        # Combine all identifiers for matching
        combined_identifiers = f"{field_id} {field_name} {automation_id}".lower()
        
        # Map field characteristics to environment variables
        field_mappings = {
            # Name fields
            'first': os.getenv('REGISTRATION_FIRST_NAME', ''),
            'last': os.getenv('REGISTRATION_LAST_NAME', ''),
            'name': os.getenv('REGISTRATION_FIRST_NAME', '') + ' ' + os.getenv('REGISTRATION_LAST_NAME', ''),
            
            # Contact fields
            'email': os.getenv('REGISTRATION_EMAIL', ''),
            'phone': os.getenv('REGISTRATION_PHONE', ''),
            'mobile': os.getenv('REGISTRATION_PHONE', ''),
            'telephone': os.getenv('REGISTRATION_PHONE', ''),
            
            # Address fields
            'address': os.getenv('LOCATION', ''),
            'street': os.getenv('LOCATION', ''),
            'city': os.getenv('LOCATION', ''),
            'postal': os.getenv('LOCATION', ''),
            'zip': os.getenv('LOCATION', ''),
            ...
            'employer': os.getenv('CURRENT_COMPANY', ''),
            'role': os.getenv('CURRENT_ROLE', ''),
            'position': os.getenv('CURRENT_ROLE', ''),
            'title': os.getenv('CURRENT_ROLE', ''),
            'experience': os.getenv('YEARS_EXPERIENCE', ''),
            'skill': os.getenv('PRIMARY_SKILLS', ''),
            
            # Education fields
            'education': os.getenv('EDUCATION_MASTERS', ''),
            'degree': os.getenv('EDUCATION_MASTERS', ''),
            'university': os.getenv('EDUCATION_MASTERS', ''),
            'school': os.getenv('EDUCATION_MASTERS', ''),
            
            # Source fields
            'source': os.getenv('JOB_BOARD', 'LinkedIn'),
            'referral': os.getenv('JOB_BOARD', 'LinkedIn'),
            'hear': os.getenv('JOB_BOARD', 'LinkedIn'),
        ...
        # Try to match field characteristics with mappings
        for keyword, value in field_mappings.items():
            if keyword in combined_identifiers:
                if value:  # Only return if we have a value
                    return value
        ...
        elif 'date' in combined_identifiers:
            # Return today's date in common format
            from datetime import datetime
            return datetime.now().strftime('%m/%d/%Y')
        
        # Default fallback values based on common patterns
        if any(word in combined_identifiers for word in ['previous', 'worked', 'employee']):
            return 'No'
        elif any(word in combined_identifiers for word in ['authorization', 'authorized']):
            return 'Yes'
        elif any(word in combined_identifiers for word in ['sponsor', 'sponsorship']):
            return 'No'
        elif any(word in combined_identifiers for word in ['citizen', 'citizenship']):
            return 'Yes'
        
        return ''  # Return empty if no match found
    
    async def _fill_required_field(self, page: Page, element, field_type: str, value: str, field_id: str) -> bool:
        """Fill a specific required field with the determined value"""
        
        try:
            if field_type in ['text', 'email', 'tel', 'password', 'input']:
                # Text input fields
                await element.clear()
                ...
                # Radio button - try to find the right option
                radio_name = await element.get_attribute('name')
                if radio_name:
                    # Find all radio buttons with the same name
                    radio_group = await page.query_selector_all(f'input[name="{radio_name}"]')
                    ...
                    if radio_group:
                        await radio_group[0].click(force=True)
                        return True
                
            elif field_type == 'checkbox':
                # Checkbox - check if value suggests it should be checked
                should_check = value.lower() in ['yes', 'true', '1', 'checked', 'on']
    ...
    async def _fill_remaining_my_info_fields(self, page: Page):
        """Fill only the specific fields that resume autofill doesn't handle"""
        print("    📝 Filling remaining My Information fields...")
        ...
        try:
            ...
            # Fill source--source field
            await self._fill_source_field(page)
            
            # Fill phone device type
            await self._fill_phone_device_type(page)
            
            # Fill radio buttons (like previous employee questions)
            await self._fill_radio_buttons(page)
            
            print("    ✅ Remaining My Information fields filled")
            
        except Exception as e:
            print(f"    ❌ Error filling remaining fields: {str(e)}")
    
    async def _fill_source_field(self, page: Page):
        """Fill the source--source dropdown field"""
        print("      🔍 Looking for source field...")
        
        source_selectors = [
            'select[id="source--source"]',
            'select[id*="source"]',
            'select[name*="source"]',
            '[data-automation-id="source--source"]'
        ]
        
        source_value = os.getenv('JOB_BOARD', 'Company Website')
        
        for selector in source_selectors:
            try:
                element = await page.wait_for_selector(selector, timeout=3000, state='visible')
                if element:
                    print(f"      ✅ Found source field: {selector}")
                    ...
                    # Try to match the source value
                    matched_option = self._match_source_option(source_value, option_texts)
                    
                    if matched_option:
                        await element.select_option(label=matched_option)
                        print(f"      ✅ Selected source: {matched_option}")
                        return
                    else:
                        # Select first non-empty option as fallback
                        if len(option_texts) > 1:
                            await element.select_option(index=1)
                            print(f"      ✅ Selected fallback source: {option_texts[1]}")
                            return
        ...
        print("      ⚠️ Source field not found")
    
    async def _fill_phone_device_type(self, page: Page):
        """Fill the phone device type dropdown"""
        print("      🔍 Looking for phone device type field...")
        
        phone_device_selectors = [
            'select[id*="phoneNumber--phoneType"]',
            'select[id*="phoneDeviceType"]',
            'select[name*="phoneType"]',
            'select[data-automation-id*="phoneNumber--phoneDeviceType"]'
        ]
        
        device_type = 'Mobile'  # Default to Mobile
        
        for selector in phone_device_selectors:
            try:
                element = await page.wait_for_selector(selector, timeout=3000, state='visible')
                if element:
                    print(f"      ✅ Found phone device type field: {selector}")
                    ...
                    # Try to find Mobile, Home, or Work
                    for option_text in option_texts:
                        if device_type.lower() in option_text.lower():
                            await element.select_option(label=option_text)
                            print(f"      ✅ Selected phone device type: {option_text}")
                            return
                    
                    # Fallback to first non-empty option
                    if len(option_texts) > 1:
                        await element.select_option(index=1)
                        print(f"      ✅ Selected fallback device type: {option_texts[1]}")
                        return
        ...
        print("      ⚠️ Phone device type field not found")
    
    async def _fill_radio_buttons(self, page: Page):
        """Fill radio buttons like previous employee questions"""
        print("      🔍 Looking for radio button fields...")
        ...
        radio_patterns = [
            {
                ...
                'selectors': [
                    'input[data-automation-id*="previousWorker"]',
                    'input[name*="previousWorker"]',
                    'input[data-automation-id*="candidateIsPreviousWorker"]'
                ],
                'value': 'No'
            },
            {
                'name': 'referral',
                'selectors': [
                    'input[data-automation-id*="referral"]',
                    'input[name*="referral"]'
                ],
                'value': 'No'
            }
        ...
        for pattern in radio_patterns:
            await self._fill_single_radio_group(page, pattern['name'], pattern['selectors'], pattern['value'])
    
    async def _fill_single_radio_group(self, page: Page, field_name: str, selectors: list, desired_value: str):
        """Fill a single radio button group"""
        print(f"      🔍 Looking for {field_name} radio buttons...")
        
        for selector in selectors:
            try:
                # Find all radio buttons with this selector
                radio_buttons = await page.query_selector_all(selector)
                
                if radio_buttons:
                    print(f"      ✅ Found {len(radio_buttons)} {field_name} radio buttons")
                    
                    # Try to find the desired value
                    for radio in radio_buttons:
                        try:
                            # Get the value or check nearby text
                            ...
                            if radio_id:
                                ...
                                if label:
                                    label_text = await label.inner_text()
                                    if desired_value.lower() in label_text.lower():
                                        await radio.click(force=True)
                                        print(f"      ✅ Selected {field_name}: {label_text}")
                                        return
                            
                            # Check value attribute
                            if value and desired_value.lower() in value.lower():
                                await radio.click(force=True)
                                print(f"      ✅ Selected {field_name}: {value}")
                                return
                                
                        except Exception as e:
                            print(f"      ⚠️ Error checking radio button: {str(e)}")
                            continue
                    
                    # If no match found, select first radio button as fallback
                    try:
                        await radio_buttons[0].click(force=True)
                        print(f"      ✅ Selected fallback {field_name} option")
                        return
                    except:
                        pass
                        
            except Exception as e:
                print(f"      ⚠️ Error with {field_name} selector {selector}: {str(e)}")
                continue
        
        print(f"      ⚠️ {field_name} radio buttons not found")
    
    def _match_source_option(self, source_value: str, available_options: list) -> str:
        """Match source value with available dropdown options"""
        if not available_options:
            return None
        
        # Try exact match first
        for option in available_options:
            if option.lower() == source_value.lower():
                return option
        
        # Try partial matches
        source_mappings = {
            'linkedin': ['LinkedIn', 'Professional Network', 'Social Media'],
            'indeed': ['Indeed', 'Job Board', 'Job Site'],
            'glassdoor': ['Glassdoor'],
            'company website': ['Company Website', 'Website', 'Direct', 'Company Site'],
            'referral': ['Referral', 'Employee Referral', 'Friend', 'Colleague'],
            'recruiter': ['Recruiter', 'Headhunter', 'Agency'],
            'other': ['Other', 'Search Engine', 'Google']
        }
        
        source_lower = source_value.lower()
        for key, variations in source_mappings.items():
        ...
        for option in available_options:
            if option.strip() and option.lower() not in ['select', 'choose', '']:
                return option
